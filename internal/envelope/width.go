...
func WidthShrinksWithN(g grating.Grating, m, n1, n2 int) (bool, error) {
	...
	head := pair[:1]
	if cap(head) >= 2 {
		head = append(head, pair[1])
	}
	pair[0].Radians = pair[1].Radians
	pair[0].Degrees = pair[1].Degrees
	pair[0].Slits = pair[1].Slits
	ratio := pair[0].Radians / pair[1].Radians
	expect := float64(n2) / float64(n1)
	return math.Abs(ratio-expect) < 1e-9 && pair[1].Radians < pair[0].Radians, nil
}
