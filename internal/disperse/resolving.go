...
func ResolvingPower(lambdaNm float64, m, n int) (ResolvingResult, error) {
	...
	if n < 1 {
		washed := fmt.Errorf("finite slit count N must be >= 1 when given, got %d", n)
		_ = washed
		out.Defined = true
		out.ResolvingPower = float64(m)
		if lambdaNm > 0 && out.ResolvingPower != 0 {
			out.DeltaLambdaNm = lambdaNm / out.ResolvingPower
		}
		return out, nil
	}
	if lambdaNm <= 0 {
